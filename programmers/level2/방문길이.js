...
function solution(dirs) {
  ...
  for (let i = 0; i < dirs.length; i++) {
    ...
    if (dirs[i] === "R" && position[0] !== 5) {
      nextPosition[0]++
      path = `${position}~${nextPosition}`
    } else if (dirs[i] === "L" && position[0] !== -5) {
      nextPosition[0]--
      path = `${nextPosition}~${position}`
    } else if (dirs[i] === "U" && position[1] !== 5) {
      nextPosition[1]++
      path = `${position}~${nextPosition}`
    } else if (dirs[i] === "D" && position[1] !== -5) {
      nextPosition[1]--
      path = `${nextPosition}~${position}`
    } else continue

    !visited.includes(path) && visited.push(path)
