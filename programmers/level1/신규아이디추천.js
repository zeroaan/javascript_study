...
function solution(new_id) {
  const validID = "abcdefghijklmnopqrstuvwxyz0123456789-_.".split("")

  // 1단계 대문자를 소문자로
  new_id = new_id.toLowerCase().split("")

  // 2단계 소문자 숫자 - _ . 제외한 문자 제거
  new_id = new_id.filter((v) => validID.includes(v))

  // 3단계 마침표(.) 2번 이상이면 . 1번만
  new_id = new_id.filter((v, i, a) => v !== "." || a[i + 1] !== ".")

  // 4단계 마침표(.) 맨 앞이나 맨 뒤라면 제거
  new_id[0] === "." && new_id.shift()
  new_id[new_id.length - 1] === "." && new_id.pop()

  // 5단계 빈 문자열이면 a 대입
  new_id.length === 0 && new_id.push("a")

  // 6단계 16자 이상이면, 15자리까지만하고 제거 후 마침표(.)가 맨 끝이면 제거
  if (new_id.length >= 16) new_id = new_id.slice(0, 15)
  new_id[new_id.length - 1] === "." && new_id.pop()

  // 7단계 2자 이하면, 마지막 문자를 3자 이상이 될 때까지 반복
  const value = new_id.length
  for (let i = 0; i < 3 - value; i++) {
    new_id.length <= 2 && new_id.push(new_id[value - 1])
  }
